...
# Index of arguments concerning the core models architecture
MODEL_ARCHITECTURE = {
    'model_type',
    'emsize',
    'rnn_type',
...
# Index of arguments concerning the models optimizer/training
MODEL_OPTIMIZER = {
    'optimizer',
    'fix_embeddings',
    'learning_rate',
...
def add_model_args(parser):
    ...
    model.add_argument('--nproplayers', type=int, default=1,
                          help='Number of propagating layers')
    ...
    model.add_argument('--beta_cyclic', type='bool', default=True,
                         help='Cyclic beta for kl loss')
    model.add_argument('--beta_cyclic_period', type=int, default=5000,
                         help='Cyclic beta period for kl loss')
    model.add_argument('--beta_cyclic_max', type=float, default=1.0,
                         help='max beta per cycle for kl loss')
    model.add_argument('--beta_cyclic_min', type=float, default=0.0,
                         help='min beta per cycle for kl loss')
    model.add_argument('--beta_annealing_max', type=float, default=1.0,
                         help='max beta for annealing kl beta')
    model.add_argument('--beta_annealing_steps', type=int, default=200000,
                         help='steps for annealing kl beta')
    model.add_argument('--gaussian', type='bool', default=False,
                         help='Use gaussian prior for kl loss')
    model.add_argument('--auto_regressive', type='bool', default=False,
                         help='Use autoregressive encoder for summary')
    model.add_argument('--mean_latent', type='bool', default=False,
                         help='Use mean of tokens as latent space')
...
def get_model_args(args):
    """Filter args for models ones.
    From a args Namespace, return a new Namespace with *only* the args specific
    to the models architecture or optimization. (i.e. the ones defined here.)
    """
...
def override_model_args(old_args, new_args):
    ...
    Decide which models args to keep and which to override when resolving a set
    of saved args and new args.
    We keep the new optimization or RL setting, and leave the models architecture alone.
    """
