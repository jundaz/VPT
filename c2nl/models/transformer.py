...
class Encoder(nn.Module):
    def __init__(self,
        ...
        self.transformer = TransformerEncoder(num_layers=args.nlayers,
                                              ...
                                              num_prop_layers=args.nproplayers)
    ...
    def forward(self,
                ...
                summ_pad_mask=None,
                std_scale=1.0):
        layer_outputs, _, mu, logvar = self.transformer(input,
                                                        input_len,
                                                        code_cls,
                                                        summ_emb,
                                                        summ_pad_mask,
                                                        std_scale)  # B x seq_len x h
        if self.use_all_enc_layers:
        ...
        return memory_bank, layer_outputs, mu, logvar
...
class Transformer(nn.Module):
    ...
    def __init__(self, args, tgt_dict):
        ...
        self.transform_cls = nn.Linear(768, self.embedder.enc_input_size)
    ...
    def _run_forward_ml(self,
        ...
        code_rep = self.embedder(code_word_rep,
                                 ...
                                 code_type_rep,
                                 mode='encoder')
        cls_transform = self.transform_cls(code_cls_rep)
        ...
        memory_bank, layer_wise_outputs, mu, logvar = self.encoder(code_rep,
                                                                   code_len,
                                                                   cls_transform,
                                                                   summ_emb=summ_emb,
                                                                   summ_pad_mask=summ_pad_mask)  # B x seq_len x h
        ...
        # exponential = logvar - torch.pow(mu, 2) - torch.exp(logvar) + 1
        # kl_loss = -0.5 * torch.sum(exponential, tuple(range(1, len(exponential.shape))))
        # kl_loss = self.kl_loss(post_mu, post_logvar, torch.zeros_like(post_mu), torch.zeros_like(post_logvar))
        prior_mu = cls_transform
        # print("using gaussian prior", self.gaussian)
        if self.gaussian:
            # print("using gaussian prior")
            prior_mu = torch.zeros_like(mu)
        kl_loss = self.kl_loss(mu, logvar, prior_mu, torch.zeros_like(logvar))

        loss['ml_loss'] = ml_loss.mean()
        loss['kl_loss'] = kl_loss.mean()
        loss['loss'] = ml_loss.mean() + kwargs['beta'] * kl_loss.mean()
        loss['loss_per_token'] = ml_loss.div((summ_len - 1).float()).mean() + kl_loss.div((summ_len - 1).float()).mean()
    ...
    def __generate_sequence(self,
        ...
        for idx in range(params['max_len'] + 1):
            ...
            if choice == 'greedy':
                ...
                # Getting the log probabilities of the chosen tokens
                log_prob = torch.log(prediction.gather(1, tgt) + 1e-20)
            else:
                assert False
    ...
    def decode(self,
        ...
        word_rep = self.embedder(code_word_rep,
                                 ...
                                 mode='encoder')
        cls_transform = self.transform_cls(code_cls_rep)
        memory_bank, layer_wise_outputs, mu, logvar = self.encoder(word_rep,
                                                                   code_len,
                                                                   cls_transform,
                                                                   std_scale=kwargs['std_scale'])  # B x seq_len x h
        params = dict()
        params['memory_bank'] = memory_bank
        params['layer_wise_outputs'] = layer_wise_outputs
        params['mu'] = mu
        params['logvar'] = logvar
