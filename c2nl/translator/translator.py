...
class Translator(object):
    ...
    def translate_batch(self, batch_inputs, std_scale=1.0):
        # Eval mode
        self.model.network.eval()

        code_word_rep = batch_inputs['code_word_rep']
        code_cls_rep = batch_inputs['code_cls_rep']
        ...
        cls_transform = model.transform_cls
        ...
        transformed_code_cls = cls_transform(code_cls_rep)
        if model_name == 'Transformer':
            memory_bank, layer_wise_outputs, mu, logvar = encoder(code_rep,
                                                                  code_len,
                                                                  code_cls=transformed_code_cls,
                                                                  std_scale=std_scale)
